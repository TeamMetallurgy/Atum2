...
public class StonewardenModel<T extends StonewardenEntity> extends ListModel<T> { //Copied from IronGolemModel, with a few changes
    ...
    public void setupAnim(@Nonnull T stonewarden, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.stonewardenHead.getYRot() = netHeadYaw * 0.017453292F;
        this.stonewardenHead.getXRot() = headPitch * 0.017453292F;
        this.stonewardenLeftLeg.getXRot() = -1.5F * this.triangleWave(limbSwing, 13.0F) * limbSwingAmount;
        this.stonewardenRightLeg.getXRot() = 1.5F * this.triangleWave(limbSwing, 13.0F) * limbSwingAmount;
        this.stonewardenLeftLeg.getYRot() = 0.0F;
        this.stonewardenRightLeg.getYRot() = 0.0F;
    }
    ...
    public void prepareMobModel(T stonewarden, float limbSwing, float limbSwingAmount, float partialTickTime) {
        ...
        if (attackTimer > 0) {
            this.stonewardenRightArm.getXRot() = -2.0F + 1.5F * this.triangleWave((float) attackTimer - partialTickTime, 10.0F);
            this.stonewardenLeftArm.getXRot() = -2.0F + 1.5F * this.triangleWave((float) attackTimer - partialTickTime, 10.0F);
        } else {
            this.stonewardenRightArm.getXRot() = (-0.2F + 1.5F * this.triangleWave(limbSwing, 13.0F)) * limbSwingAmount;
            this.stonewardenLeftArm.getXRot() = (-0.2F - 1.5F * this.triangleWave(limbSwing, 13.0F)) * limbSwingAmount;
        }
    }
