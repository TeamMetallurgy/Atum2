...
public class QuernRecipe extends RotationRecipe<QuernTileEntity> {
    ...
    public QuernRecipe(Ingredient input, @Nonnull ItemStack output, int rotations) {
        this(new ResourceLocation(Atum.MOD_ID, "quern_" + Objects.requireNonNull(ForgeRegistries.ITEMS.getKey(input.getItems()[0].getItem())).getPath() + "_to_" + Objects.requireNonNull(ForgeRegistries.ITEMS.getKey(output.getItem())).getPath() + (output.getCount() > 1 ? "_" + output.getCount() : "")), input, output, rotations);
    }
