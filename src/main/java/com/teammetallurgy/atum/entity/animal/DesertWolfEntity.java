...
public class DesertWolfEntity extends TamableAnimal implements PlayerRideableJumping, ContainerListener, MenuProvider, NeutralMob {
    ...
    public static AttributeModifierMap.MutableAttribute getAttributes() {
        return MobEntity.createMobAttributes().add(Attributes.MAX_HEALTH, 20.0F).add(Attributes.MOVEMENT_SPEED, 0.4D).add(Attributes.ATTACK_DAMAGE, 8.0D).add(JUMP_STRENGTH);
    }
    ...
    public static boolean canSpawn(EntityType<? extends DesertWolfEntity> animal, IServerWorld world, SpawnReason spawnReason, BlockPos pos, Random random) {
        return pos.getY() > 62 && ((ServerWorld) world.getChunkSource().getLevel()).getGameRules().getBoolean(GameRules.RULE_DOMOBSPAWNING) && world.canSeeSkyFromBelowWater(pos) && AtumEntities.canAnimalSpawn(animal, world, spawnReason, pos, random);
    }
    ...
    public void travel(@Nonnull Vector3d travelVec) {
        if (this.isAlive()) {
            if (this.isVehicle() && this.canBeControlledByRider() && this.isSaddled()) {
                ...
                if (livingBase != null) {
                    this.yRot = livingBase.yRot;
                    this.yRotO = this.yRot;
                    this.xRot = livingBase.xRot * 0.5F;
