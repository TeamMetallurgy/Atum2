            ...
            if (this.getItemBySlot(EquipmentSlot.MAINHAND).getItem() == AtumItems.GREATSWORD_IRON) {
                ...
                if (attackEntity) {
                    if (knockback > 0.0F && entity instanceof LivingEntity) {
                        ((LivingEntity) entity).knockback(knockback * 0.5F, Mth.sin(this.yRot * ((float) Math.PI / 180F)), -Mth.cos(this.yRot * ((float) Math.PI / 180F)));
                        entity.push(-Mth.sin(this.yRot * (float) Math.PI / 180.0F) * knockback * 0.5F, 0.1D, Mth.cos(this.yRot * (float) Math.PI / 180.0F) * knockback * 0.5F);
