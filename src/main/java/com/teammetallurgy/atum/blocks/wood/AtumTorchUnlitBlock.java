...
public class AtumTorchUnlitBlock extends AtumTorchBlock {
    private static final List<AtumTorchBlock> TORCHES = Arrays.asList(AtumBlocks.PALM_TORCH, AtumBlocks.DEADWOOD_TORCH, AtumBlocks.LIMESTONE_TORCH, AtumBlocks.BONE_TORCH, AtumBlocks.PHARAOH_TORCH);
    private static final Map<Block, Block> UNLIT = Maps.newHashMap();
    private static final Map<Block, Block> LIT = Maps.newHashMap();

    private AtumTorchUnlitBlock() {
        super();
        this.setLightLevel(0);
    }

    public static void registerUnlitTorches() {
        for (AtumTorchBlock torch : TORCHES) {
    ...
    public boolean onBlockActivated(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult rayTraceResult) {
        ...
        if ((heldStack.getItem() instanceof ItemFlintAndSteel || block.getLightValue(block.getDefaultState(), world, pos) > 0)) {
            ...
            world.setBlockState(pos, getLitTorch(this).getStateForPlacement(world, pos, state.get(AtumTorchBlock.FACING), hitX, hitY, hitZ, 0, player, hand));
