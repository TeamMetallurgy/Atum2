...
public abstract class AtumCarver<C extends ICarverConfig> extends WorldCarver<C> {
    ...
    @Override
    protected boolean func_230358_a_(@Nonnull IChunk chunk, @Nonnull Function<BlockPos, Biome> biomePos, BitSet bitSet, @Nonnull Random rand, @Nonnull BlockPos.Mutable pos, @Nonnull BlockPos.Mutable pos1, @Nonnull BlockPos.Mutable pos2, int i, int i1, int i2, int x, int z, int i5, int y, int i7, @Nonnull MutableBoolean mutableBoolean) {
        int bit = i5 | i7 << 4 | y << 8;
        if (bitSet.get(bit)) {
            return false;
        } else {
                ...
                if (y < 11) {
                    ...
                    if (mutableBoolean.isTrue()) {
                        ...
                        if (chunk.getBlockState(pos2).getBlock() == AtumBlocks.SAND) {
                            chunk.setBlockState(pos2, biomePos.apply(pos).getGenerationSettings().getSurfaceBuilderConfig().getTop(), false);
                        }
                    }
                }
                return true;
