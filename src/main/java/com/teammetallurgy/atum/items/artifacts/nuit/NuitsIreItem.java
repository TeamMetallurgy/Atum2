...
public class NuitsIreItem extends KhopeshItem {
    ...
    private static void applyWither(LivingEntity attacker, LivingEntity target, boolean isNuitsQuarterHeld) {
        if (attacker != target) {
            for (int l = 0; l < 8; ++l) {
                Atum.proxy.spawnParticle(AtumParticles.Types.NUIT_WHITE, target, target.posX + (random.nextDouble() - 0.5D) * (double) target.width, target.posY + random.nextDouble() * (double) target.height, target.posZ + (random.nextDouble() - 0.5D) * (double) target.width, 0.0D, 0.0D, 0.0D);
            }
