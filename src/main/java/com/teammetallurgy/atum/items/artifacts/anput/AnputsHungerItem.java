...
public class AnputsHungerItem extends DaggerItem {
    ...
    public static void onAttack(AttackEntityEvent event) {
        ...
        if (event.getTarget() instanceof LivingEntity && player.getHeldItem(EnumHand.MAIN_HAND).getItem() == AtumItems.ANPUTS_HUNGER) {
            player.getFoodStats().addStats(1, 0.0F);
            ((LivingEntity) event.getTarget()).addPotionEffect(new PotionEffect(MobEffects.NAUSEA, 40));
            hungerTimer = 200;
        }
    }
